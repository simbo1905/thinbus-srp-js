...
[ ![Build Status](https://codeship.com/projects/f95bffe0-3b5d-0133-b993-428ee47fa127/status?branch=master)](https://codeship.com/projects/102093)
...
Typically cryptography depends on secure random numbers and historically this has been a weak point with browsers. Thinbus tries to use the `WebCryptoAPI` secure random number API in modern browsers. If that is not available it falls back to a pure javascript isaac random number generator which is warmed up upon page load. The reasoning as to why I consider this acceptable is as follows. 
...
1. Fix to issue #3 that Java generated verifiers (e.g. you server generated it and emailed out a temporary password for the user to do a first login) had a 1/12 chance of not working with the browser. This changes the computation of 'x' on the browser to drop leading zeros to be consistent with how things are handled everwhere else (i.e. nimbus is java so drops the left zeros). Unfortunately this means that there is a 1/16 chance that any users who generated verifiers using the browser on prior release will not longer be able to login against any verifier generated by JS in a previous release so they will have to do a password reset. 
