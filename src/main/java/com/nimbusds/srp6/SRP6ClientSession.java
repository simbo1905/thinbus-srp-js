...
public class SRP6ClientSession extends SRP6Session {
	...
	public static enum State {
		...
		/**
		 * The session is initialised and ready to begin authentication by
		 * proceeding to {@link #STEP_1}.
		 */
		INIT,

		/**
		 * The authenticating user has input their identity 'I' (username) and
		 * password 'P'. The session is ready to proceed to {@link #STEP_2}.
		 */
		STEP_1,

		/**
		 * The user identity 'I' is submitted to the server which has replied
		 * with the matching salt 's' and its public value 'B' based on the
		 * user's password verifier 'v'. The session is ready to proceed to
		 * {@link #STEP_3}.
		 */
		STEP_2,

		/**
		 * The client public key 'A' and evidence message 'M1' are submitted and
		 * the server has replied with own evidence message 'M2'. The session is
		 * finished (authentication was successful or failed).
		 */
		STEP_3
	}
	 ...
	 *
	 * @param routine
	 *            The password key 'x' routine or {@code null} to use the
	...
	public SRP6ClientCredentials step2(final SRP6CryptoParams config,
			final BigInteger s, final BigInteger B) throws SRP6Exception {
		...
		if (digest == null)
			throw new IllegalArgumentException(
					"Unsupported hash algorithm 'H': " + config.H);
		...
		// Check timeout
		if (hasTimedOut())
			throw new SRP6Exception("Session timeout",
					SRP6Exception.CauseType.TIMEOUT);

		// Check B validity
		if (!SRP6Routines.isValidPublicValue(config.N, B))
			throw new SRP6Exception("Bad server public value 'B'",
		...
		if (xRoutine != null) {
			...
			x = xRoutine.computeX(config.getMessageDigestInstance(),
					s.toByteArray(), userID.getBytes(Charset.forName("UTF-8")),
		...
		if (clientEvidenceRoutine != null) {
			...
			// With custom routine
			SRP6ClientEvidenceContext ctx = new SRP6ClientEvidenceContext(
					userID, s, A, B, S);
	...
	public void step3(final BigInteger M2) throws SRP6Exception {
		...
		if (M2 == null)
			throw new IllegalArgumentException(
					"The server evidence message 'M2' must not be null");
		...
		if (serverEvidenceRoutine != null) {
			...
			// With custom routine
			SRP6ServerEvidenceContext ctx = new SRP6ServerEvidenceContext(A,
					M1, S);

			computedM2 = serverEvidenceRoutine.computeServerEvidence(config,
					ctx);
		...
		if (!computedM2.equals(M2))
			throw new SRP6Exception("Bad server credentials",
					SRP6Exception.CauseType.BAD_CREDENTIALS);
